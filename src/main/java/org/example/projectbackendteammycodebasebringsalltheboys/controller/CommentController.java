...
public class CommentController {
    ...
    public ResponseEntity<List<CommentResponse>> getCommentsByAssignment(
        ...
        boolean isCreator = assignment.getCreator() != null
                && assignment.getCreator().getId().equals(currentUser.getId());

        boolean isEnrolled = assignment.getCourse() != null
                && assignment.getCourse().getSchoolClass() != null
                && classEnrollmentService.isUserInClass(
                currentUser,
                assignment.getCourse().getSchoolClass());

        if (!isCreator && !isEnrolled) {
            throw new AccessDeniedException("You are not authorized to view comments for this assignment");
        }
