...
class AuthControllerTest {
  ...
  @Test
  @DisplayName(
      "POST /auth/register with validation errors returns register view without calling service")
  ...
  @Test
  @DisplayName(
      "POST /auth/register when service throws IllegalStateException returns register view with error message")
  void registerUser_serviceThrowsIllegalStateException_returnsRegisterViewWithError()
      throws Exception {
  ...
  void loginPage_returnsLoginView() throws Exception {
    mockMvc.perform(get("/auth/login")).andExpect(status().isOk()).andExpect(view().name("login"));
  }
  ...
  void loginPage_withRegisteredParam_returnsLoginView() throws Exception {
    mockMvc
        .perform(get("/auth/login").param("registered", ""))
        .andExpect(status().isOk())
  ...
  void logoutSuccess_returnsLogoutSuccessView() throws Exception {
    mockMvc
        .perform(get("/auth/logout-success"))
        .andExpect(status().isOk())
