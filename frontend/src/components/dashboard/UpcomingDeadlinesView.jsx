...
export function UpcomingDeadlinesView() {
  ...
  const getStatusBadge = (status) => {
    switch (status) {
      case 'ASSIGNED': return <Badge variant="outline">Inte inlämnad</Badge>;
      case 'TURNED_IN': return <Badge variant="secondary">Inlämnad</Badge>;
      case 'EVALUATED': return <Badge variant="default">Rättad</Badge>;
      default: return null;
    }
  };
        ...
        <div className="space-y-4">
          {deadlines.length === 0 ? (
            ...
            deadlines.map((item) => {
              ...
              const isUrgent = date.getTime() - new Date().getTime() < 48 * 60 * 60 * 1000;

              return (
                <div key={item.assignmentId} className="flex items-center justify-between border-b border-muted pb-3 last:border-0 last:pb-0">
                  <div className="space-y-1">
                    <Link to={`/assignments/${item.assignmentId}`} className="text-sm font-semibold hover:underline flex items-center gap-2">
                      {item.title}
