...
const EMPTY_S3_KEYS = Object.freeze([]);
...
export function FileSection({
    ...
    const files = useMemo(() => {
        const serverFileIds = new Set(initialFiles.map(f => f.id));
        const uniqueLocalFiles = localFiles.filter(f => !serverFileIds.has(f.id));
        return [...initialFiles, ...uniqueLocalFiles];
    ...
    const handleUpload = async (e) => {
        ...
        try {
            ...
            setLocalFiles((prev) => [...prev, savedFile]);
        ...
        <>
            <Card>
                ...
                <CardContent className="space-y-4">
                    {uploadError && (
                        <div
                            className="text-sm text-destructive flex items-center justify-between bg-destructive/10 p-2 rounded">
                            {uploadError}
                            <Button variant="ghost" size="icon" className="h-4 w-4" aria-label="Stäng felmeddelande"
                                ...
                                >
                                    <div className="flex items-center gap-3 overflow-hidden">
                                        ...
                                        <div className="flex flex-col overflow-hidden">
                                            ...
                                            <span
                                                className="text-xs text-muted-foreground">{formatFileSize(file.fileSize)}</span>
                                        </div>
                                    </div>
                                    <div className="flex items-center gap-1">
                                        {isImage(file) && (
                                            <Button variant="ghost" size="icon"
                                                    aria-label={`Förhandsvisa ${file.fileName}`}
                                                    onClick={() => handlePreview(file)}>
                                                <ZoomIn className="h-4 w-4"/>
                                            </Button>
                                        )}
                                        <Button variant="ghost" size="icon" aria-label={`Ladda ner ${file.fileName}`}
